        ...
        {airdrops.length > 0 ? (
          <div className="grid md:grid-cols-3 gap-6">
            {airdrops.map((airdrop) => (
              <motion.div
                key={airdrop.id}
                className="bg-gray-800 p-5 rounded-lg shadow-lg flex flex-col items-center text-center transition-transform hover:scale-105"
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
                transition={{ duration: 0.5 }}
              >
                ...
                <button className="mt-4 bg-yellow-500 hover:bg-yellow-600 text-black font-bold py-2 px-4 rounded-full transition-transform hover:scale-105">
                  Claim Now
                </button>
              </motion.div>
            ))}
